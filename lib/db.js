...
// Tabela de arquivos anexados (certidões e licenças) — conteúdo em base64.
let arquivosReady = global.__oleArquivosReady;
async function ensureArquivos() {
  await ensureSchema();
  if (arquivosReady) return arquivosReady;
  arquivosReady = pool.query(`
  ...
  `);
  global.__oleArquivosReady = arquivosReady;
  return arquivosReady;
}

module.exports = { pool, ensureSchema, ensureArquivos, getAllState, getStateVersion, setState, importState };
